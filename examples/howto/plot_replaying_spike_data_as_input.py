...
net_A.add_evoked_drive(
    **drive_params,
    weights_ampa=weights["ampa"],
    weights_nmda=weights["nmda"],
...
with tempfile.TemporaryDirectory() as tmp_dir:
    ...
    print(
        f"Extracted {sum(len(t) for t in cell_response.spike_times)} spikes from Network A"
    )
...
print(
    f"Number of valid pyramidal cells with spikes to be used for input: {len(spike_data)}"
)
...
if drive_name in net_B.external_drives:
    ...
    drive_connections = sum(
        1 for conn in net_B.connectivity if conn["src_type"] == drive_name
    )
