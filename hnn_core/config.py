...
class Config(object):
    ...
    Attributes
    ----------
    cfg : NetPyNE SimConfig object
    net : NetPyNE NetParams object
    """

    def __init__(self, params):
        self.cfg = _fill_netpyne_cfg()
...
def _fill_netpyne_cfg():
    ...
    # True # leave as False to avoid extra printouts
    ...
    cfg.distributeSynsUniformly = True  # one syn per section
    cfg.allowSelfConns = False  # allow connections from a cell to itself
    cfg.allowConnsWithWeight0 = False  # make faster
    cfg.oneSynPerNetcon = False  # allow the same synapse for multiple netcons

    # ----------------------------------------------------------------------------
    # Recording
    # ----------------------------------------------------------------------------
    cfg.recordTraces = {'V_soma': {'sec': 'soma', 'loc': 0.5, 'var': 'v'}}
    cfg.recordCells = [('L2Basket', 0), ('L2Pyr', 0), ('L5Basket', 0),
                       ('L5Pyr', 0)]
    ...
    evprox = ['evokedProximal_1_L2Basket', 'evokedProximal_1_L2Pyr',
              'evokedProximal_1_L5Basket', 'evokedProximal_1_L5Pyr',
              'evokedProximal_2_L2Basket', 'evokedProximal_2_L2Pyr',
              'evokedProximal_2_L5Basket', 'evokedProximal_2_L5Pyr']
    evdist = ['evokedDistal_1_L2Basket', 'evokedDistal_1_L2Pyr',
              'evokedDistal_1_L5Basket', 'evokedDistal_1_L5Pyr']

    popColors = {'L2Basket': [0.0, 0.0, 0.0], 'L2Pyr': [0.0, 0.6, 0.0],
                 'L5Basket': [0.0, 0.0, 1.0], 'L5Pyr': [1.0, 0.0, 0.0],
                 'Evoked proximal': [0.0, 1.0, 1.0],
                 'Evoked distal': [1.0, 1.0, 0.0]}

    cfg.analysis['iplotTraces'] = {'include': [('L5Pyr', 0)],
                                   'oneFigPer': 'cell', 'saveFig': False,
                                   'showFig': True,
                                   'timeRange': [0, cfg.duration]}

    cfg.analysis['iplotRaster'] = {'include': pops, 'showFig': True,
                                   'popColors': popColors,
                                   'markerSize': 6, 'orderInverse': True}

    cfg.analysis['iplotSpikeHist'] = {'include': [*pops, evprox, evdist,
                                                  'extRhythmicProximal',
                                                  'extRhythmicDistal'],
                                      'legendLabels': pops +
                                      ['Evoked proximal',
                                       ...
                                       'Rhythmic distal'],
                                      'popColors': popColors,
                                      'yaxis': 'count', 'showFig': True}

    cfg.analysis['iplotDipole'] = {'showFig': True}

    # change freq to 40 for alpha&beta tut
    cfg.analysis['iplotDipolePSD'] = {'showFig': True, 'maxFreq': 80}

    # change freq to 40 for alpha&beta tut
    cfg.analysis['iplotDipoleSpectrogram'] = {'showFig': True,
                                              'maxFreq': 80}

    cfg.analysis['plotConn'] = {'includePre': pops, 'includePost': pops,
                                'feature': 'strength'}
    ...
    # ----------------------------------------------------------------------------
    #
    # HNN parameters in original GUI but not in config files
    # (adapted to NetPyNE)
    #
    # ----------------------------------------------------------------------------
    ...
    # firing threshold (sets netParams.defaultThreshold)
    cfg.threshold = 0.0
    ...
    # current values
    # not sure where these distal values come from, need to check
    ...
    # increment (ms) for avg evoked input start
    # (for trial n, avg start time is n * evinputin)
    cfg.sync_evinput = 1,  # do evoked inputs hit cells at same time
    ...
    cfg.f_max_spec = 40.
    cfg.dipole_scalefctr = 30e3  # scale factor for dipole
    # based on scaling needed to match model ongoing rhythms from
    # Jones 2009 - for ERPs can use 300
    # for ongoing rhythms + ERPs ... use ... ?
    cfg.dipole_smooth_win = 15.0  # window for smoothing (box filter)
    # 15 ms from jones 2009; shorter
    ...
    # prng_state is a string for a filename containing the random state
    # one wants to use
