...
def test_extracellular_api(fix_net_model, request):
    ...
    net, _ = net_model(add_drives_from_params=True)
    ...
    rec_arr = ExtracellularArray(good_positions, times=[0], voltages=[[[0], [0]]])
    ...
    assert z_delta == 2000
    with pytest.raises(
        ValueError, match="Electrode array positions must contain more than 1 contact"
    ):
        _, _ = _get_laminar_z_coords([(1, 2, 3)])
    with pytest.raises(
        ValueError, match="Make sure the electrode positions are equispaced, colinear"
    ):
...
def test_transmembrane_currents(fix_net_model, request):
    ...
    net_model = request.getfixturevalue(fix_net_model)
    net, _ = net_model(add_drives_from_params=True, reduced=True)
    ...
    _ = simulate_dipole(net, tstop=40.0)
    assert_allclose(net.rec_arrays["net_Im"].voltages, 0, rtol=1e-10, atol=1e-10)
...
def test_transfer_resistance():
    ...
    for seg_idx in range(sec.nseg):
        ...
        var_r_psa = np.sqrt(
            elec_pos[0] ** 2 + (elec_pos[1] - seg_ctr_pts[seg_idx]) ** 2
        )
        target_vals["psa"].append(1000 / (4.0 * np.pi * conductivity * var_r_psa))
        ...
        target_vals["lsa"].append(
            ...
            * np.log(
            ...
            / (4.0 * np.pi * conductivity * 2 * seg_lens[seg_idx])
        )
...
def test_extracellular_backends(fix_net_model, fix_run_simulation, request):
    ...
    assert (
        len(electrode_array["arr1"])
        == len(joblib_net.rec_arrays["arr1"].positions)
        == len(mpi_net.rec_arrays["arr1"].positions)
    )
    assert (
        len(joblib_net.rec_arrays["arr1"]) == len(mpi_net.rec_arrays["arr1"]) == 2
    )  # length == n.o. trials
...
def test_rec_array_calculation(fix_net_model, request):
    ...
    assert (
        len(net.rec_arrays["arr1"]) == len(net.rec_arrays["arr1"].voltages) == 1
    )  # n_trials
    assert len(net.rec_arrays["arr1"].voltages[0]) == 4  # n_contacts
    assert len(net.rec_arrays["arr1"].voltages[0][0]) == len(
        net.rec_arrays["arr1"].times
    )
    ...
    for trial_idx in range(n_trials):
        ...
        assert_allclose(
            net.rec_arrays["arr1"]._data[trial_idx][1],
