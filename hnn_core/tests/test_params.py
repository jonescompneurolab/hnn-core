...
def test_remove_nulled_drives(tmp_path):
    ...
    assert all(
        [drive not in net_removed.external_drives.keys() for drive in drives_removed]
    )
    ...
    assert all([drive not in net_removed.pos_dict.keys() for drive in drives_removed])
...
class TestConvertToJson:
    ...
    def test_default_network_connectivity(self, tmp_path):
        ...
        convert_to_json(self.path_default, outpath)
    ...
    def test_convert_to_json_legacy(self, tmp_path):
        ...
        # Download params
        param_url = (
            "https://raw.githubusercontent.com/hnnsolver/"
    ...
    def test_convert_to_json_bad_type(self):
        ...
        with pytest.raises(ValueError, match="Unrecognized extension, expected one of"):
            convert_to_json(good_path, path_str)

        # Bad params_fname
        with pytest.raises(
            TypeError, match="params_fname must be an instance of str or Path"
        ):
