...
def test_cell_response(tmp_path):
    ...
    with pytest.warns(
        DeprecationWarning, match='Writing cell response to txt files is ' 'deprecated'
    ):
    ...
    with pytest.warns(
        DeprecationWarning,
        match='Reading cell response from txt files is ' 'deprecated',
    ):
    ...
    # size small
    assert list(cell_response.__dict__.keys()) == sim_attributes + net_attributes
    ...
    with pytest.raises(TypeError, match='spike_times should be a list of lists'):
        cell_response = CellResponse(
            spike_times=[1, 2], spike_gids=spike_gids, spike_types=spike_types
        )
    ...
    with pytest.raises(
        ...
        cell_response = CellResponse(
            spike_times=[[2.3456, 7.89]], spike_gids=spike_gids, spike_types=spike_types
        )
    ...
    with pytest.raises(
        TypeError, match=r'spike_types\[ev\] must be a list\. ' r'Got int\.'
    ):
    ...
    with pytest.raises(
        ValueError,
        match='gid_ranges should contain only ' 'disjoint sets of gid values',
    ):
        gid_ranges = {
