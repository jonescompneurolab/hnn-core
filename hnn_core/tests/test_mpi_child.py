...
def test_read_stderr():
    ...
    stderr.flush()
    proc_data_bytes = _read_stderr(pipe_stderr_r, selectors.EVENT_READ)
    assert proc_data_bytes == "test_data".encode()

    stderr.write("@")
    stderr.flush()
    with pytest.raises(ValueError, match="Invalid signal start"):
        _read_stderr(pipe_stderr_r, selectors.EVENT_READ)
    ...
    stderr.flush()
    proc_data_bytes, data_len = _read_stderr(pipe_stderr_r,
                                             selectors.EVENT_READ)
    assert data_len == 1000
    assert proc_data_bytes == "blahblahblah".encode()
...
def test_read_stdout():
    ...
    with io.StringIO() as buf_out, redirect_stdout(buf_out):
        _read_stdout(pipe_stdout_r, selectors.EVENT_READ)
        output = buf_out.getvalue()
    assert output == "Test output"

    stdout.write("end_of_sim")
    stdout.flush()
    signal = _read_stdout(pipe_stdout_r, selectors.EVENT_READ)
...
def test_child_run():
    ...
    with MPISimulation(skip_mpi_import=True) as mpi_sim:
        ...
        # use _read_stderr to get data_len + data
        _, data_len = _read_stderr(pipe_stderr_r, selectors.EVENT_READ)
...
def test_empty_data():
    ...
    with pytest.raises(RuntimeError, match="MPI simulation didn't return any "
                       "data"):
        _process_child_data(data_bytes, expected_len)
