'''
TODO: This if from https://github.com/espenhgn/iCSD, 
in the future we may need to implement the following algorithms on our own.
...
class CSD(object):
    ...
    def filter_csd(self, csd, filterfunction='convolve'):
        ...
        try:
            ...
            raise ae("{} not equal to 'filtfilt' or 'convolve'".format(
                filterfunction))
    ...
    def __init__(self,
        ...
        if vaknin_el:
            ...
            if lfp.ndim == 1:
                ...
                self.lfp = np.empty(
                    (lfp.shape[0] + 2, lfp.shape[1])) * lfp.units
            self.lfp[0, ] = lfp[0, ]
            self.lfp[1:-1, ] = lfp
            self.lfp[-1, ] = lfp[-1, ]
        else:
...
class DeltaiCSD(CSD):
    ...
    def __init__(self,
        ...
        try:
            ...
            print('units of coord_electrode ({}) and diam ({}) differ'.format(
        ...
        try:
            ...
            print(
                'diam must be positive scalar or of same shape as coord_electrode'
            )
            raise ae
        if diam.size == 1:
        ...
        self.f_matrix = np.empty(
            (self.coord_electrode.size, self.coord_electrode.size))
        ...
        for j in range(el_len):
            for i in range(el_len):
                ...
                f_cyl0 = si.quad(self._f_cylinder,
                                 ...
                                 epsabs=self.tol)[0]
                f_cyl1 = si.quad(self._f_cylinder,
                                 a=lower_int,
                                 b=upper_int,
                                 args=(-float(self.coord_electrode[j]),
                                       float(self.diam[j]), float(self.sigma)),
                ...
                mom = (self.sigma - self.sigma_top) / (self.sigma +
                                                       self.sigma_top)
...
class SplineiCSD(CSD):
    ...
    def get_f_matrix(self):
        ...
        for j in range(el_len):
            for i in range(el_len):
                f_mat0[j, i] = si.quad(self._f_mat0,
                                       a=z_js[i],
                                       b=z_js[i + 1],
                                       args=(z_js[j + 1], float(self.sigma),
                                             float(self.diam[j])),
                                       epsabs=self.tol)[0]
                f_mat1[j, i] = si.quad(self._f_mat1,
                                       a=z_js[i],
                                       b=z_js[i + 1],
                                       args=(z_js[j + 1], z_js[i],
                                             ...
                                             float(self.diam[j])),
                                       epsabs=self.tol)[0]
                f_mat2[j, i] = si.quad(self._f_mat2,
                                       a=z_js[i],
                                       b=z_js[i + 1],
                                       args=(z_js[j + 1], z_js[i],
                                             float(self.sigma),
                                             float(self.diam[j])),
                                       epsabs=self.tol)[0]
                f_mat3[j, i] = si.quad(self._f_mat3,
                                       a=z_js[i],
                                       b=z_js[i + 1],
                                       args=(z_js[j + 1], z_js[i],
                                             float(self.sigma),
    ...
    def get_csd(self):
        ...
        if self.lfp.ndim == 1:
            ...
            cs_lfp = np.vstack(
                (np.zeros(self.lfp.shape[1]), np.asarray(self.lfp),
    ...
    def _calc_k_matrix(self):
        ...
        return np.dot(
            np.linalg.inv(
                np.dot(c_jm1, tjm1) + 2 * np.dot(c_jm1, tj0) + 2 * c_jall +
                np.dot(c_j0, tjp1)), 3 *
            (np.dot(np.dot(c_jm1, c_jm1), tj0) -
             np.dot(np.dot(c_jm1, c_jm1), tjm1) + np.dot(
                 np.dot(c_j0, c_j0), tjp1) - np.dot(np.dot(c_j0, c_j0), tj0)))
...
def estimate_csd(lfp,
    ...
    csd = neo.AnalogSignal(csd_pqarr.T,
                           t_start=lfp.t_start,
                           sampling_rate=lfp.sampling_rate)
    csd_filtered = neo.AnalogSignal(csd_pqarr_filtered.T,
                                    t_start=lfp.t_start,
                                    sampling_rate=lfp.sampling_rate)
