...
class HNNGUI:
    ...
    Parameters
    ----------
    theme_color : str, default = "#802989"
        The theme color of the whole dashboard. By default, "#802989".
    total_height : int, default = 800
        The height of the GUI (in pixels, same for all following parameters). By default, 800.
    total_width : int, default = 1300
        The width of the GUI. By default, 1300.
    header_height : int, default = 50
        The height of the header. By default, 50.
    button_height : int, default = 30
        The height of buttons. By default, 30.
    operation_box_height : int, default = 60
        The operation_box_height of operations box. By default, 60.
    drive_widget_width : int, default = 200
        The width of drive widget. By default, 200.
    left_sidebar_width : int, default = 576
        The width of left sidebar. By default, 576.
    log_window_height : int, default = 150
        The height of logging window. By default, 150.
    status_height : int, default = 30
        The height of status bar. By default, 30.
    dpi : int, default = 96
        The screen dpi. By default, 96.
    ...
    def compose(self, return_layout=True):
        ...
        Parameters
        ----------
        return_layout : bool, default = True
            If the method returns the layout object which can be rendered by
            IPython.display.display() method. By default, True. 
        """
        box_style = """
            style="
                background: gray;
    ...
    def _simulate_upload_data(self, file_url):
        """Simulate uploading data by setting the value of the load_data_button.
    ...
    def load_drive_and_connectivity(self):
        """Add drive and connectivity ipywidgets from params using the 'add_connectivity_tab' 
        function. This function also adds the drive tab using the 'add_drive_tab' function."""
        with self._log_out:
            # Add connectivity
    ...
    def add_drive_tab(self, params):
        """
        Add drive tab based on provided parameters. This function converts the provided
        parameters into a Network object, creates dict objects for each drive, and
        stores parameters for non-tonic drives. It then adds drive widgets to the GUI
        ...
        params : dict
            Dictionary containing network parameters including external drives
            and biases. This dict will get converted into a Network object.       
...
def create_expanded_button(
    ...
    This function creates a button widget with the given description, style,
    layout, and other properties.
    ...
    button_style : str
        The style of the button (e.g., "primary", "secondary").
    layout : Layout
        The layout properties for the button.
    disabled : bool, default = False
        Whether the button is disabled.
    button_color : str, default = "#8A2BE2"
...
def _get_connectivity_widgets(conn_data):
    ...
    conn_data : dict
        Dictionary containing connectivity data with receptor names as keys and
        their corresponding properties (e.g., weight, location, source and target GIDs).
    
    Returns
    -------
    sliders : list
        A list of VBox widgets, each containing a receptor name and its associated properties.
    """
...
def _get_drive_weight_widgets(layout, style, location, data=None):
    """
    Create drive weight and delay widgets for specified cell types and location.
    ...
    style : dict
        Style options for widgets
    location : str
        Location of the drive input. Accepts "proximal" or "distal". Distal inputs
        exclude the L5 basket cell.
    data : dict, optional
        Nested dictionary to override default weights and delays.

    Returns
    -------

    widgets_list : list
        List of widgets (HTML headers + BoundedFloatText) for display in GUI.
    widgets_dict : dict
        Dictionary containing weights and delays for both receptor types and each cell type.
    """
    default_data = {
        "weights_ampa": {
            "L5_pyramidal": 0.0,
...
def _get_rhythmic_widget(
    ...
    """
    Create rhythmic drive widgets for specified parameters.
    ...
    tstop_widget : ipywidget
        Widget that specifies the stop time of the simulation.
    layout : dict
        Layout options for widgets.
    style : dict
        Style options for widgets.
    location : str
        ...
        exclude the L5 basket cell.
    data : dict, optional
        Nested dictionary to override default rhythmic drive parameters.
    weights_ampa : dict, optional
        Nested dictionary to override default AMPA weights.
    weights_nmda : dict, optional
        Nested dictionary to override default NMDA weights.
    delays : dict, optional
        Nested dictionary to override default synaptic delays. 
    n_drive_cells : int, optional
        Number of drive cells to override default value.
    cell_specific : bool, optional
        Cell specificity to override default value.
    
    Returns
    -------

    drive : dict
        Dictionary containing the rhythmic drive parameters and associated widgets.
    drive_box : VBox
        VBox widget containing the rhythmic drive widgets for display in GUI.
    """
    default_data = {
        "tstart": 0.0,
        "tstart_std": 0.0,
...
def _get_poisson_widget(
    ...
    """
    Create poisson drive widgets for specified parameters.
    ...
    drive : dict
        Dictionary containing the poisson drive parameters and associated widgets.
    drive_box : VBox
        VBox widget containing the poisson drive widgets for display in GUI.
    """
    default_data = {
        "tstart": 0.0,
        "tstop": tstop_widget.value,
...
def _get_evoked_widget(
    ...
    """
    Create evoked drive widgets for specified parameters.
    ...
    style : dict
        Style options for widgets.
    location : str
        Location of the drive input. Accepts "proximal" or "distal". Dist
        inputs exclude the L5 basket cell.
    data : dict, optional
        Nested dictionary to override default evoked drive parameters.
    weights_ampa : dict, optional
        Nested dictionary to override default AMPA weights.
    weights_nmda : dict, optional
        Nested dictionary to override default NMDA weights.
    ...
    -------
    drive : dict
        Dictionary containing the evoked drive parameters and associated widgets.
    drive_box : VBox
        VBox widget containing the evoked drive widgets for display in GUI.
    """
...
def _get_tonic_widget(name, tstop_widget, layout, style, data=None):
    """
    Create tonic drive widgets for specified parameters.
    ...
    style : dict
        Style options for widgets.
    data : dict, optional
        Nested dictionary to override default tonic drive parameters.

    Returns
    -------
    drive : dict
        Dictionary containing the tonic drive parameters and associated widgets.
    drive_box : VBox
        VBox widget containing the tonic drive widgets for display in GUI.
    """
...
def _build_drive_objects(
    ...
    """
    Create drive widgets for specified parameters and drive type.
    
    Parameters
    ----------
    
    drive_type : str
        The type of drive to create. Options are "Rhythmic", "Bursty", "Poisson",
        "Evoked", "Gaussian", and "Tonic".
    name : str
        The name of the drive.    
    tstop_widget : ipywidget
        Widget that specifies the stop time of the simulation.
    ...
    location : str
        ...
        inputs exclude the L5 basket cell.
    drive_data : dict
        Nested dictionary to override default drive parameters.
    weights_ampa : dict
        Nested dictionary to override default AMPA weights.
    weights_nmda : dict
        Nested dictionary to override default NMDA weights.
    ...
    drive : dict
        Dictionary containing the drive parameters and associated widgets.
    drive_box : VBox
        VBox widget containing the drive widgets for display in GUI.  
    """
...
def add_network_connectivity_tab(net, connectivity_out, connectivity_textfields):
    ...
    connectivity_out : ipywidget.Output
        Output widget to display the connectivity tab.
    connectivity_textfields : list
        List to store the connectivity text field widgets.

    Returns
    -------

    net : Network
        The same initial constructed network object.
    """
