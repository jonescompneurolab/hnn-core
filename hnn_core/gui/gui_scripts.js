...
/* ----------------------------------------------------------------------
    Add tab classes automatically

    note: there's not an easy way to add classes to these tabs that
          are automatically generated by ipywidgets in the python
          code itself, so we do so in javascript instead
    ---------------------------------------------------------------------- */

(function () {
    // function to apply classes to all tabs
    function updateTabClasses() {
        const tabs = document.querySelectorAll(".lm-TabBar-tab");

        tabs.forEach(tab => {
            const label = tab.querySelector(".lm-TabBar-tabLabel")?.textContent?.trim();
            if (!label) return;

            const cls = "tab-" + label.toLowerCase().replace(/\s+/g, "-");
            
            // only add if the class isn't already present
    ...
    // watch for changes in the tab bar to catch tab switches
    const tabBars = document.querySelectorAll(".lm-TabBar");

    if (tabBars.length > 0) {
        const observer = new MutationObserver((mutationsList) => {
            for (const mutation of mutationsList) {
                // ipywidgets changes attributes on tab activation, so we're looking
                // for that
        ...
        // observe each tab bar for attribute changes on child tabs
