...
def _check_drive_parameter_values(drive_type, **kwargs):
    if 'tstop' in kwargs:
        if kwargs['tstop'] is not None:
            if kwargs['tstop'] < 0.0:
                raise ValueError(
                    f'End time of {drive_type} drive cannot be ' 'negative'
                )
            if 'tstart' in kwargs and kwargs['tstop'] < kwargs['tstart']:
                raise ValueError(
                    f'Duration of {drive_type} drive cannot be ' 'negative'
                )
    if 'sigma' in kwargs:
        if kwargs['sigma'] < 0.0:
...
def _create_extpois(*, t0, T, lamtha, prng):
    ...
    if t0 < 0:
        raise ValueError(
            'The start time for Poisson inputs must be' f'greater than 0. Got {t0}'
        )
    if T < t0:
        raise ValueError(
